...
export default function WrappedFooter({ views, volume }) {
  const totalMessages = volume ? (volume.total_sent || 0) + (volume.total_received || 0) : 0
  
  const enhancementPrompt = totalMessages > 0
  ...
  const { enhancement, loading } = useEnhancement(enhancementPrompt, !!enhancementPrompt)
