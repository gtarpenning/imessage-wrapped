...
export async function getCompletion(prompt) {
  ...
  console.log('→ Calling OpenAI API for prompt:', prompt.substring(0, 60) + '...')
      ...
      input: [
        {
          type: 'message',
          role: 'system',
          content: 'You are a witty, playful assistant that enhances statistics with short, fun commentary. Keep responses under 15 words. Be clever and conversational.'
        },
        {
          type: 'message',
          role: 'user',
  ...
  if (!completion) {
    completion = data.output?.[0]?.content || data.choices?.[0]?.message?.content || ''
  }
  
  // Ensure completion is always a string
  if (typeof completion !== 'string') {
    console.error('❌ [LLM] Completion is not a string! Type:', typeof completion, 'Value:', completion)
    completion = ''
  }
